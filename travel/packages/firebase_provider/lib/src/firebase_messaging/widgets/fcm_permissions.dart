    ...
    return Column(
      children: [
        row(
          ...
          statusMap[_settings.authorizationStatus] ?? 'Getting error ...!!!',
        ),
        if (defaultTargetPlatform == TargetPlatform.iOS) ...[
          row(
            'Alert',
            settingsMap[_settings.alert] ?? 'Getting error ...!!!',
          ),
          row(
            'Announcement',
            settingsMap[_settings.announcement] ?? 'Getting error ...!!!',
          ),
          row(
            'Badge',
            settingsMap[_settings.badge] ?? 'Getting error ...!!!',
          ),
          row(
            'Car Play',
            settingsMap[_settings.carPlay] ?? 'Getting error ...!!!',
          ),
          row(
            'Lock Screen',
            settingsMap[_settings.lockScreen] ?? 'Getting error ...!!!',
          ),
          row(
            'Notification Center',
            settingsMap[_settings.notificationCenter] ?? 'Getting error ...!!!',
          ),
          row(
            'Show Previews',
            previewMap[_settings.showPreviews] ?? 'Getting error ...!!!',
          ),
          row(
            'Sound',
            settingsMap[_settings.sound] ?? 'Getting error ...!!!',
          ),
        ],
        ElevatedButton(
